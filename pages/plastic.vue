  ...
  <div dir="rtl" class="min-h-screen bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-6 sm:py-8 md:py-10">
      <div class="text-center sm:text-right">
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold mb-2">🪑 البلاستيك والتخزين</h1>
        <p class="text-gray-500 text-sm sm:text-base mb-6 sm:mb-8 md:mb-10">علب حفظ طعام - كراسي - ترابيزات - سلات قمامة - جراكن مياه</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 lg:gap-8">
        <ProductCard
          v-for="product in plasticProducts"
          :key="product.id"
          :product="product"
      ...
      <!-- Empty State -->
      <div v-if="!loading && plasticProducts.length === 0" class="text-center py-16 sm:py-20">
        ...
        <p class="text-gray-400 text-sm sm:text-base mt-2">سيتم إضافة منتجات جديدة قريباً</p>
...
const loadProducts = async () => {
  ...
  if (error) {
    console.error("Error loading plastic products:", error)
  } else {
    products.value = data || []
  }
  loading.value = false
...
const plasticProducts = computed(() => products.value)
